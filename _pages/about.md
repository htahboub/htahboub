    ...
    var keyHandler = function (event) {
        if (pattern.indexOf(event.key) >= 0 && event.key === pattern[current]) {
            ...
            if (pattern.length === current) {
                ...
                document.getElementsByClassName("mapmyvisitors")[0].classList.toggle('hidden');
            }
        } else {
            current = 0;
        }
...
<script>document.getElementsByClassName("mapmyvisitors")[0].classList.toggle('hidden');</script>
